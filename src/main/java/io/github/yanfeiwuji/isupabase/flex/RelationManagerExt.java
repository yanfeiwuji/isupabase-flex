...
public class RelationManagerExt {

    private RelationManagerExt() {
    }
    ...
    public static void setIgnoreRelations(Set<String> ignoreRelations) {
        RelationManagerExt.ignoreRelations.set(ignoreRelations);
    }

    public static <T> void addIgnoreRelations(LambdaGetter<T>... ignoreRelations) {
        Set<String> relations = RelationManagerExt.ignoreRelations.get();
        if (relations == null) {
    ...
    public static void setQueryRelations(Set<String> queryRelations) {
        RelationManagerExt.onlyQueryRelations.set(queryRelations);
    }

    public static <T> void addQueryRelations(LambdaGetter<T>... queryRelations) {
        Set<String> relations = RelationManagerExt.onlyQueryRelations.get();
        if (relations == null) {
            relations = new HashSet<>();
    ...
    public static List<AbstractRelation> getRelations(Class<?> clazz) {
        return MapUtil.computeIfAbsent(classRelations, clazz, RelationManagerExt::doGetRelations);
    }
        ...
        try {
            relations.forEach(relation -> {
                ...
                try {
                    ...
                    // 通过中间表关联查询
                    if (StringUtil.isNotBlank(relation.getJoinTable())) {
                        ...
                        if (selfFieldValues.size() > 1) {
                            ...
                            queryWrapper
                                    .where(column(relation.getJoinSelfColumn()).eq(selfFieldValues.iterator().next()));
                        }
            ...
            relations.forEach(relation -> {
                ...
                try {
                    ...
                    if (CollectionUtil.isNotEmpty(targetObjectList)) {
                        ...
                        doQueryRelationsWithDepthRelQuery(mapper, targetObjectList, currentDepth + 1, maxDepth,
                                ignoreRelations,
                                queryRelations);
