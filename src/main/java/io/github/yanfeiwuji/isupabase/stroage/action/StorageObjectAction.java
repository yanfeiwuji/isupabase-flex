...
public class StorageObjectAction {
    ...
    public StorageShortInfo update(@PathVariable String bucketId, HttpServletRequest request) {
        ...
        final String key = Optional.ofNullable(request).map(HttpServletRequest::getServletPath)
        ...
        return new StorageShortInfo(storageObject.getId(), key);
    }


    @PostMapping("/list/{bucketId}")
    public List<StorageShortInfo> list(@PathVariable String bucketId,
                                       @RequestBody ObjectListParam param
    ) {
        // todo add query
        //
//        QueryWrapper.create().from(STORAGE_OBJECT)
//                .where(STORAGE_OBJECT.BUCKET_ID.eq(bucketId))
//                .limit(Optional.ofNullable(param).map(ObjectListParam::limit).orElse(100L))
//                .offset();


// pgrstDb.selectListByQuery(storageObjectMapper, )
        return List.of();
    }
